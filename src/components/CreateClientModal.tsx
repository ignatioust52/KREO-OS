...
export default function CreateClientModal({ isOpen, onClose, businessId }: { isOpen: boolean, onClose: () => void, businessId: string }) {
  ...
  async function handleSubmit(formData: FormData) {
    ...
    const type = formData.get('type') as 'INDIVIDUAL' | 'BUSINESS';
    const email = formData.get('email') as string;
    const phone = formData.get('phone') as string;
    
    const result = await createClient({ name, type, email, phone });
      ...
      <div className="bg-kreo-panel w-full max-w-md rounded-2xl shadow-xl overflow-hidden flex flex-col max-h-[90vh]">
        ...
        <form action={handleSubmit} className="p-6 overflow-y-auto">
          ...
          <div className="space-y-4">
            <div>
              <label className="block text-sm font-medium mb-1">Client Name / Business Name *</label>
            ...
            <div>
              ...
              <select name="type" className="w-full px-4 py-2 border border-kreo-ink/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-kreo-amber">
            ...
            <div>
              ...
              <input name="email" type="email" className="w-full px-4 py-2 border border-kreo-ink/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-kreo-amber" />
            </div>
            
            <div>
              <label className="block text-sm font-medium mb-1">Phone</label>
              <input name="phone" type="tel" className="w-full px-4 py-2 border border-kreo-ink/20 rounded-lg focus:outline-none focus:ring-2 focus:ring-kreo-amber" />
            </div>
          </div>
